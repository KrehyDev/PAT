    ...
    convertButton.addEventListener('click', () => {
      ...
      for (const line of lines) {
        if (line.startsWith('*')) {
          ...
          html += `<div style="text-align: left"><img id="leftimg" src="${imageUrl}" alt="ObrÄÄzek v levo"></div>`;
        }
        else if (line.startsWith('[[File:') && line.endsWith('|right]]')) {
          const imageUrl = line.slice(7, -8);
          html += `<img id="rightimg" src="${imageUrl}" alt="ObrÄÄzek v pravo">`;
        }
        else if (line.startsWith('[[File:') && line.endsWith(']]')) {
          const imageUrl = line.slice(7, -2);
          html += `<img id="centerimg" src="${imageUrl}" alt="ObrÄÄzek">`;
        }
